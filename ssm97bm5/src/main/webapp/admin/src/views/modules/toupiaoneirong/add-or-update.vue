<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="投票主题" prop="toupiaozhuti">
          <el-select :disabled="ro.toupiaozhuti" v-model="ruleForm.toupiaozhuti" placeholder="请选择投票主题">
            <el-option
        ...
        <div v-else>
          <el-form-item class="input" label="投票主题" prop="toupiaozhuti">
	      <el-input v-model="ruleForm.toupiaozhuti"
                placeholder="投票主题" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="24">  
...
export default {
  ...
  methods: {
	...
	this.ruleForm.neirong = this.ruleForm.neirong.replace(reg,'../../../ssm97bm5/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `toupiaoneirong/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.toupiaoneirongCrossAddOrUpdateFlag = false;
						 this.parent.search();
